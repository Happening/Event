...
exports.dayToString = (day, noYear) ->
	d = new Date(day*864e5)
	dayNames[d.getUTCDay()]+' '+d.getUTCDate()+' '+monthNames[d.getUTCMonth()]+(if noYear then '' else ' '+d.getUTCFullYear())

exports.timeToString = (time) ->
	time = time % 86400 # wrap around 25:05 -> 01:05
	if !time?
		tr("None")
	else if time<0
		tr("All day")
